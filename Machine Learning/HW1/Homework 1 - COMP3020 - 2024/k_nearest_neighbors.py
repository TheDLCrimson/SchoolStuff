...
class KNNClassifier(NearestNeighborClassifier):
    ...
    def predict(self, X_test):
        ...
        for sample in X_test:
            distances = np.linalg.norm(self.X - sample, ord = self.ord, axis = 1) 
...
class EpsilonballNearestNeighborClassifier(NearestNeighborClassifier):
    ...
    def predict(self, X_test):
        ...
        for sample in X_test:
            distances = np.linalg.norm(self.X - sample, ord=self.ord, axis=1)
            within_epsilon = distances <= self.epsilon
            selected = self.y[within_epsilon]

            if len(selected) == 0:
                prediction = -1
            else:
                val, count = np.unique(selected, return_counts=True)
                prediction = val[np.argmax(count)].astype(int)
